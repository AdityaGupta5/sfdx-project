trigger SendConfirmationEmail on Session_Speaker__c (after insert) {
    ...
    for(Session_Speaker__c sessionSpeaker : trigger.New) {
    	sessionSpeakerIds.add(sessionSpeaker.Id);  
    }
    // Retrieve session name and time + speaker name and email address related to ID's
    // Retrieves only if speaker has email address
    List<Session_Speaker__c> sessionSpeakerList = [Select Id, Name, Session__r.Name, Session__r.Session_Date__c, Speaker__r.First_Name__c,
                                                   Speaker__r.Last_Name__c, Speaker__r.Email__c
                                                   From Session_Speaker__c
                                                   Where Id IN :sessionSpeakerIds
                                                   AND Speaker__r.Email__c <> NULL];
    if(sessionSpeakerList != Null && sessionSpeakerList.size() > 0) {
        String [] addresses = new String[]{},
                  subjects = new String[]{},
                  messages = new String[]{};
                      
        for(Session_Speaker__c sessionSpeaker: sessionSpeakerList)	{
